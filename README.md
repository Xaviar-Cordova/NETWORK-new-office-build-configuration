...
Devices' capabilities later, We then created VLANs to segment how data is transferred so there are fewer chances of eavesdropping & VLAN 3 itself has been set with a management interface enabled on each switch so we can SSH remote into those later. since we're introducing VLAN, we implemented MSTP so each of them will have a better idea on what paths to choose, but kept all VLAN in group 0 for now. Just to be safe no random Cisco switch overwrites all the others one day, we also set VTP to transparent mode. finally, it is only a matter of setting up the trunk and assigning ports to its intended segments.
...
pretending it'll process on it). We wanted a Windows server to hand out DHCP leases, so we set the Helper address to 10.0.3.254 right after enabling the sub-interfaces for the VLANs.
